...
TEST_CASE("Class ContactGraph")
{
    ...
    SECTION("load two square A,B. A's normal is [0,0,1], B's normal is [0,0,-1]")
    {
        ...
        REQUIRE(std::abs(contactPolygon.ComputeArea() - 1) < 1e-4);
    }
    ...
    SECTION("Scaling Error"){
        pPolyMesh A, B;
        A = make_shared<PolyMesh>(varList);
        B = make_shared<PolyMesh>(varList);

        boost::filesystem::path current_path(boost::filesystem::current_path());

        boost::filesystem::path strA = current_path / "../data/Mesh/contact_scaling_bug/01.obj";
        bool texturedModel;
        A->ReadOBJModel(strA.string().c_str(), texturedModel, false);

        boost::filesystem::path strB = current_path / "../data/Mesh/contact_scaling_bug/02.obj";
        B->ReadOBJModel(strB.string().c_str(), texturedModel, false);

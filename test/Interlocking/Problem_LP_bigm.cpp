...
problem_LP_bigm::problem_LP_bigm() {
    x_sol = {0,0,0,1,1,1};            // initial position vector that becomes the solution
    big_m = -5E4;
...
/********************************************************************************/
/****BIG M WITH UNIDIMENSIONAL LAMBDA********************************************/
/********************************************************************************/

problem_LP_bigm_unidim::problem_LP_bigm_unidim() {
    x_sol = {0,0,0,1};            // initial position vector that becomes the solution
    big_m = -5E4;
    hessian_triplets_nb = 0;
};

// destructor
problem_LP_bigm_unidim::~problem_LP_bigm_unidim() = default;

// returns the problem dimensions
bool problem_LP_bigm_unidim::get_nlp_info(Index &n, Index &m, Index &nnz_jac_g, Index &nnz_h_lag, IndexStyleEnum &index_style) {
    n = 4;                          // 3 variables + 3 extra params for the bigM
    m = 3;                          // 3 inequalities
    nnz_jac_g = 8;                  // non zero elements in Jacobian
    nnz_h_lag = 0;                  // non-zero elements in Lagrangian Hessian
    index_style = TNLP::C_STYLE;    // use the C style indexing (0-based)
...
// returns the variable bounds
bool problem_LP_bigm_unidim::get_bounds_info(Index n, Number *x_l, Number *x_u, Index m, Number *g_l, Number *g_u) {
    // Ensure dimensions are set properly
...
// returns the initial point for the problem
bool problem_LP_bigm_unidim::get_starting_point(Index n, bool init_x, Number *x, bool init_z, Number *z_L, Number *z_U,
                                          Index m, bool init_lambda, Number *lambda) {
...
// return the objective function
bool problem_LP_bigm_unidim::eval_f(Index n, const Number *x, bool new_x, Number &obj_value) {
    // minus sign everywhere, we are looking for max
    obj_value = 3 * x[0] + 4 * x[1] + 2 * x[2] + x[3] * big_m;
    obj_value = -1.0 * obj_value;
    return true;
}

// return the gradient of the objective function grad_{x} f(x)
bool problem_LP_bigm_unidim::eval_grad_f(Index n, const Number *x, bool new_x, Number *grad_f) {
...
// return the value of the constraints: g(x)
bool problem_LP_bigm_unidim::eval_g(Index n, const Number *x, bool new_x, Index m, Number *g) {
    g[0] = 2 * x[0] + x[3] * big_m;
    g[1] = 1 * x[0] + 2 * x[2] + x[3] * big_m;
    g[2] = 3 * x[1] + 1 * x[2] + x[3] * big_m;
    return true;
}

// return the triplet structure or values of the Jacobian
bool problem_LP_bigm_unidim::eval_jac_g(Index n, const Number *x, bool new_x,
                                  Index m, Index nele_jac, Index *iRow, Index *jCol, Number *values) {
    if (values == nullptr) {
        // return the 2 first triplet index row, col for the structure of the Jacobian
        #
...
//return the structure or values of the Hessian
bool problem_LP_bigm_unidim::eval_h(Index n, const Number *x, bool new_x, Number obj_factor, Index m, const Number *lambda,
                              bool new_lambda, Index nele_hess, Index *iRow, Index *jCol, Number *values) {
...
void problem_LP_bigm_unidim::finalize_solution(SolverReturn status,
                                         Index n,
                                         const Number *x,
                                         const Number *z_L,
                                         const Number *z_U,
